...
from backend.src.modules.agent.nodes.tech_node import TechNode
...
class AgentGraph:
    ...
    def the_conditional(self, state: State) -> Literal["tech_support", "billing", "hr", "other"]:
        # 3.1 flash lite
    ...
    def build(self):
        ...
        self.builder.add_node("billing_node", tech_llm.generate_response)
        self.builder.add_node("hr_node", tech_llm.generate_response)
        self.builder.add_node("other_node", tech_llm.generate_response)

        self.builder.add_edge(START, "extractor")
        self.builder.add_conditional_edges("extractor", self.the_conditional, {
